...
func init() {
	raid.RegisterDriver(api.DISK_DRIVER_AACRAID, NewAdaptecRaid)
}
...
func (raid *AdaptecRaid) GetName() string {
	return baremetal.DISK_DRIVER_AACRAID
}
...
func (ada *AdaptecRaidAdaptor) PreBuildRaid(confs []*api.BaremetalDiskConfig) error {
	// set to raid expose raw mode
	if !ada.isRaidExposeRawMode() {
...
func (ada *AdaptecRaidAdaptor) fillDevices() error {
	...
	log.Errorf("+++PD ret: %v", ret)
	devs := getPhyDevices(ada.GetIndex(), ret)
	log.Errorf("----PD phydevs: %v", devs)
...
func getPhyDevices(adapter int, lines []string) []*AdaptecRaidPhyDev {
	...
	for _, line := range lines {
		...
		log.Errorf("****line: %q, baseDev: %#v, dev: %#v", line, dev.RaidBasePhyDev, dev)
	}
	return devs
}
...
func (ada *AdaptecRaidAdaptor) setControllerMode(mode int) error {
	cmd := GetCommand("SETCONTROLLERMODE", fmt.Sprintf("%d", ada.GetIndex()), fmt.Sprintf("%d", mode))
...
func newPhyDev(adapter int) *AdaptecRaidPhyDev {
	dev := &AdaptecRaidPhyDev{
		RaidBasePhyDev: raid.NewRaidBasePhyDev(api.DISK_DRIVER_AACRAID),
	}
	dev.Adapter = adapter
	return dev
}
